...
/**
 * Solution to the Stable Marriage Problem as described
 * in "Randomized Algorithms" by Motwani and Raghavan.
...
public class StableMarriage {
    ...
    public void setManPreferences(int man, int[] preferences) {
        if ((preferences != null) && (preferences.length == _numCouples)) {
            _menPreferences[man] = preferences;
        } else {
            throw new IllegalArgumentException("Wrong size for preferences for man " + man + ": expected: " + _numCouples + ", actual: " + (preferences == null ? 0 : preferences.length));
        }
    }

    public void setWomanPreferences(int woman, int[] preferences) {
        if ((preferences != null) && (preferences.length == _numCouples)) {
            _womenPreferences[woman] = preferences;
        } else {
            throw new IllegalArgumentException("Wrong size for preferences for woman " + woman + ": expected: " + _numCouples + ", actual: " + (preferences == null ? 0 : preferences.length));
        }
    }
    ...
    public static void main(String[] args) {
        if (args.length == 0) {
            ...
            try {
                ...
                int numMen = numCouples;
                int numWomen = numCouples;

                StableMarriage sm = new StableMarriage(numCouples);

                // Create a collection that can be easily randomly shuffled.
                List<Integer> choices = new ArrayList<Integer>(numCouples);
                for (int i=0; i<numCouples; i++) {
                    choices.add(i);
                }

                // Initialize preferences for men using the random collection.
                for (int i=0; i<numMen; i++) {
                    int[] preferences = new int[numWomen];
                    Collections.shuffle(choices);
                    copy(choices, preferences);
                    sm.setManPreferences(i, preferences);
                }

                // Initialize preferences for women using the random collection.
                for (int i=0; i<numWomen; i++) {
                    int[] preferences = new int[numMen];
                    Collections.shuffle(choices);
                    copy(choices, preferences);
                    sm.setWomanPreferences(i, preferences);
                }
